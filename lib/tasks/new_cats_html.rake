...
task :newcats2html => :environment do
  ...
  @depth_0.each do |d0|
    cat_html << "
    %li.depth0
      = check_box_tag \"category_#{d0.oe_id}\", '#{d0.oe_id}', false, :name => \"oe_ids[]\"
      %label
        = '#{d0.name}'"
    ...
    @children_0.each do |d1|
      cat_html << "
        %li.hide.depth1
          = check_box_tag \"category_#{d1.oe_id}\", '#{d1.oe_id}', false, :name => \"oe_ids[]\"
          %label
            = '#{d1.name}'"
      ...
      @children_1.each do |d2|
        cat_html << "
            %li.hide.depth2
              = check_box_tag \"category_#{d2.oe_id}\", '#{d2.oe_id}', false, :name => \"oe_ids[]\"
              %label
                = '#{d2.name}'"
        ...
        @children_2.each do |d3|
          cat_html << "
                %li.hide.depth3
                  = check_box_tag \"category_#{d3.oe_id}\", '#{d3.oe_id}', false, :name => \"oe_ids[]\"
                  %label
                    = '#{d3.name}'"
        end
      end
